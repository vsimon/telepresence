...
func main() {
	dev, err := OpenTunTap(net.IP{10, 43, 0, 0}, net.IP{10, 43, 0, 1}, net.IP{0xff, 0xff, 0, 0})
	if err != nil {
...
func getTuntapComponentId() (string, error) {
	...
	for ; i < 1000; i++ {
		...
		cid := unicodeTostring(component_id)
		if cid == TUNTAP_COMPONENT_ID {
			var valtype uint32
